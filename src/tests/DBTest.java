...
class DBDriver extends TestDriver implements GlobalConst {
	...
	protected boolean test1() {
		...
		int num_read = SystemDefs.JavabaseDB.getNumPageReads();
		int num_wrote = SystemDefs.JavabaseDB.getNumPageWrites();
		...
		System.out.println("Number of Pages Read in test1: " + (SystemDefs.JavabaseDB.getNumPageReads() - num_read));
		System.out.println("Number of Pages Wrote in test1: " + (SystemDefs.JavabaseDB.getNumPageWrites() - num_wrote));
	...
	protected boolean test2() {
		...
		if (status == OK) {
			...
			for (int i = 0; i < 20 && status == OK; ++i) {
				...
				if (readStr.equals(testStr) != true) {
					status = FAIL;
					System.err.print(
							"*** Data read does not match what " + "was written on page " + (runStart.pid + i) + "\n");
		...
		System.out.println("Number of Pages Read in test2: " + (SystemDefs.JavabaseDB.getNumPageReads() - num_read));
		System.out.println("Number of Pages Wrote in test2: " + (SystemDefs.JavabaseDB.getNumPageWrites() - num_wrote));
	...
	protected boolean test4() {
		...
		if (status == OK) {
			...
			try {
				...
				e.printStackTrace();
				System.err
						.print("*** Too little space available: could not " + "allocate " + (dbsize - 3) + " pages\n");
			}
		...
		if (status == OK) {
			System.out.print("  - At this point, all pages should be claimed.  " + "Try to allocateone more.\n");
			try {
		...
		if (status == OK) {
			System.out.print("  - Free the last two pages: this tests a boundary " + "condition in the space map.\n");
			try {
